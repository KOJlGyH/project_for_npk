...
def index():
    if request.method == "GET":
        ...
        if data[0] == '{':
            filename = list(data.rstrip('}').lstrip('{').split(': ')[1].rstrip('"').lstrip('"') + '.jpg')
            del filename[filename.index('\\')]
    ...
    if request.method == 'POST':
        ...
        if data == '':
            files = request.files
            print(files)
            key = list(dict(files).keys())[0]
            print(list(files))
            files = list(dict(files).values())
            print(files)
            for i in range(len(files)):
                files[i].save(f'images/{key}{i}.jpg')
            return '0'
        elif data[1] == 'u':
            command = 'update'
        elif data[1] == 'i':
